...
!(function (y) {
  ...
  function t(t, s) {
    (this.elmt = t),
      (this.settings = y.extend({}, e, y.vegas.defaults, s)),
  ...
  (t.prototype = {
    _init: function () {
      var t,
        s,
        i = "BODY" === this.elmt.tagName,
        e = this.settings.timer,
        n = this.settings.overlay,
        o = this;
      this._preload(),
        i ||
          ((s = y('<div class="vegas-content-scrollable">')),
          (t = y('<div class="vegas-content">')
        ...
        e &&
          ...
          this.$elmt.prepend(e)),
        n &&
          ((e = y('<div class="vegas-overlay">')),
          "string" == typeof n && e.css("background-image", "url(" + n + ")"),
          (this.$overlay = e),
          this.$elmt.prepend(e)),
        this.$elmt.addClass("vegas-container"),
        i || (this.$elmt.append(s), s.append(t)),
        setTimeout(function () {
          o.trigger("init"),
            o._goto(o.slide),
            o.settings.autoplay && o.trigger("play");
        }, 1);
    },
    _preload: function () {
      for (var t = 0; t < this.settings.slides.length; t++)
    ...
    _goto: function (t) {
      ...
      var s,
        i,
        e,
        n = this.$elmt.children(".vegas-slide"),
        o = this.settings.slides[t].src,
        t = this.settings.slides[t].video,
        a = this._options("delay"),
        r = this._options("align"),
        h = this._options("valign"),
        l = this._options("cover"),
        d = this._options("color") || this.$elmt.css("background-color"),
        g = this,
        u = n.length,
        c = this._options("transition"),
        p = this._options("transitionDuration"),
        m = this._options("animation"),
        f = this._options("animationDuration");
      function v() {
        g._timer(!0),
          setTimeout(function () {
            c &&
              (g.support.transition
                ? (n
                    .css("transition", "all " + p + "ms")
                    .addClass("vegas-transition-" + c + "-out"),
                  n.each(function () {
                    var t = n.find("video").get(0);
                    t && ((t.volume = 1), g._fadeOutSound(t, p));
                  }),
                  s
                    .css("transition", "all " + p + "ms")
                    .addClass("vegas-transition-" + c + "-in"))
                : s.fadeIn(p));
            for (var t = 0; t < n.length - g.settings.slidesToKeep; t++)
              n.eq(t).remove();
      ...
      this.settings.firstTransition &&
        this.first &&
        (c = this.settings.firstTransition || c),
        this.settings.firstTransitionDuration &&
          this.first &&
          (p = this.settings.firstTransitionDuration || p),
        this.first && (this.first = !1),
        "repeat" !== l &&
          (!0 === l ? (l = "cover") : !1 === l && (l = "contain")),
        ("random" === c || c instanceof Array) &&
          (c =
            c instanceof Array
              ? this._random(c)
              : this._random(this.transitions)),
        ("random" === m || m instanceof Array) &&
          (m =
            m instanceof Array
              ? this._random(m)
        ...
        ("auto" === p || a < p) && (p = a),
        "auto" === f && (f = a),
        (s = y('<div class="vegas-slide"></div>')),
        this.support.transition && c && s.addClass("vegas-transition-" + c),
        t
          ? (((i =
              t instanceof Array ? this._video(t) : this._video(t.src)).loop =
              void 0 === t.loop || t.loop),
            (i.muted = void 0 === t.muted || t.muted),
            !1 === i.muted
              ? ((i.volume = 0), this._fadeInSound(i, p))
              : i.pause(),
            (a = y(i).addClass("vegas-video").css("background-color", d)),
            this.support.objectFit
              ? a
                  .css("object-position", r + " " + h)
                  .css("object-fit", l)
                  .css("width", "100%")
                  .css("height", "100%")
              : "contain" === l && a.css("width", "100%").css("height", "100%"),
            s.append(a))
          : ((e = new Image()),
            (t = y('<div class="vegas-slide-inner"></div>')
              .css("background-image", 'url("' + o + '")')
              .css("background-color", d)
              .css("background-position", r + " " + h)),
            "repeat" === l
              ? t.css("background-repeat", "repeat")
              : t.css("background-size", l),
            this.support.transition &&
              m &&
              t
                .addClass("vegas-animation-" + m)
                .css("animation-duration", f + "ms"),
            s.append(t)),
        this.support.transition || s.css("display", "none"),
        u ? n.eq(u - 1).after(s) : this.$elmt.prepend(s),
        n.css("transition", "all 0ms").each(function () {
          (this.className = "vegas-slide"),
            "VIDEO" === this.tagName && (this.className += " vegas-video"),
            c &&
              ((this.className += " vegas-transition-" + c),
              (this.className += " vegas-transition-" + c + "-in"));
        }),
        g._timer(!1),
        i
          ? (4 === i.readyState && (i.currentTime = 0), i.play(), v())
          : ((e.src = o), e.complete ? v() : (e.onload = v));
    },
    _end: function () {
    ...
    options: function (t, s) {
      ...
      if ("object" == typeof t)
        this.settings = y.extend({}, e, y.vegas.defaults, t);
      else {
    ...
    (y.vegas = {}),
    (y.vegas.defaults = e),
    (y.vegas.isVideoCompatible = function () {
      return !0;
    });
